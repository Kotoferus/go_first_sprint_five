...
func (r Running) Calories() float64 {
	return CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift/MInKm*r.Duration.Hours()*MinInHours
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	im := InfoMessage{}
	im.TrainingType = s.TrainingType
	im.Duration = s.Duration
	im.Distance = s.distance()
	im.Speed = s.meanSpeed()
	//im.Calories = s.Calories()
	return im
}
